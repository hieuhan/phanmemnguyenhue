...
const categoryInsert = async (category) =>
{
    let resultVar = 0;
    try 
    {
        ...
        await pool.request()  
            ...
            .execute('Categories_Insert').then(function(recordsets) {
                ...
                resultVar = output['CategoryId'];
            });
    } 
    catch (error) 
    {
        console.error(`categoryInsert error => ${error}\n`);
    }

    return resultVar;
}

const productCategoryInsert = async (productCategory) =>
{
    let resultVar = 0;
    try 
    {
        console.log('Xử lý dữ liệu ProductCategories\n');

        const pool = await poolPromise;

        await pool.request()  
            .input("ActBy", sql.NVarChar(150), configs.actionBy)  
            .input("SiteId", sql.Int, productCategory.SiteId)  
            .input("ProductId", sql.Int, productCategory.ProductId)  
            .input("CategoryId", sql.Int, productCategory.CategoryId)  
            .output('ProductCategoryId', sql.Int)
            .execute('ProductCategories_Insert').then(function(recordsets) {
                const output = (recordsets.output || {});
                resultVar = output['ProductCategoryId'];
            });
    } 
    catch (error) 
    {
        console.error(`productCategoryInsert error => ${error}\n`);
    }
...
module.exports = {
    ...
    productCategoryInsert: productCategoryInsert,
