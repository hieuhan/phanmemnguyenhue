...
const scraperObject = {
    ...
    async scraper(browser, pageUrl)
    {
        try 
        {
            ...
            const currentPage = utils.urlGetCurrentPage(pageUrl);
                ...
                try 
                {
                    ...
                    let pagePromise = (productUrl) => new Promise(async(resolve, reject) =>
                    {
                        try 
                        {
                            ...
                            await newPage.goto(productUrl, { waitUntil: 'domcontentloaded'});
                    ...
                    if(nextButtonExist)
                    {
                        ...
                        if(nextButtonParent.length > 0)
                        {
                            ...
                            {
                                const nextUrl = utils.getProductUrl(configs.websiteDomain, nextButtonParentHref);

                                const nextPage = utils.urlGetCurrentPage(nextButtonParentHref);
                    ...
                    if(breadcrumbElement.length > 0)
                    {
                        ...
                        if(breadcrumbs.length > 3)
                        {
                            ...
                            if(landTypeName.length > 0)
                            {
                                ...
                                resultVar = await database.landTypeInsert(landType);
                            }
                        }
                    }
                } 
