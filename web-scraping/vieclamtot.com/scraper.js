        ...
        try 
        {
            ...
            const parserData = async (data, pageUrl, productUrl) =>
            {
                try 
                {
                    ...
                    {
                        const [categoryId, provinceId, customerId] = await Promise.all([
                        ...
                        if(provinceId > 0)
                        {
                            ...
                            productId = await parserProduct(data, categoryId, provinceId, (districtId || 0), (wardId || 0), (streetId || 0), customerId, pageUrl, productUrl);
            ...
            const parserCategory = async (data, pageUrl, productUrl) =>
            {
                ...
                try 
                {
                    if(data.props.initialState.adView.adInfo.ad_params.job_type.value.length > 0)
                    {
                        const category = 
                        {
                            SiteId: configs.siteId,
                            Name: data.props.initialState.adView.adInfo.ad_params.job_type.value
                        }
            ...
            const parserProduct = async (data, categoryId, provinceId, districtId, wardId, streetId, customerId, pageUrl, productUrl) =>
            {
                let resultVar = 0;
                try 
                {
